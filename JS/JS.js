...
function calcularFrecuenciaCambio(cantidadVacas, cantidadOrdeños, cantidadEquipos, fechaCambio) {
    ...
    const fechaProximoCambio = new Date(
        fechaUltimoCambio.getTime() + frecuenciaDias * 24 * 60 * 60 * 1000
    );
...
function mostrarProximosCambiosEnTabla(cantidadCambios, ordenAscendente) {
    ...
    const proximosCambios = Array.from({
        length: cantidadCambios
    }, (_, i) => ({
        fecha: new Date(
            ultimoCambio.fechaProximoCambio.getTime() +
            i * ultimoCambio.frecuenciaDias * 24 * 60 * 60 * 1000
        ),
        frecuenciaDias: ultimoCambio.frecuenciaDias,
    }));

    proximosCambios.sort((a, b) =>
        ordenAscendente ? a.fecha - b.fecha : b.fecha - a.fecha
    );
    ...
    proximosCambios.forEach(cambio =>
        tbody.appendChild(createTableRow([
            cambio.fecha.toLocaleDateString("es-ES"),
            cambio.frecuenciaDias
        ]))
    );
...
document.getElementById("calculoForm").addEventListener('submit', function(event) {
    ...
    if (isValidPositiveNumber(cantidadVacas) && isValidPositiveNumber(cantidadOrdeños) && isValidPositiveNumber(cantidadEquipos)) {
        ...
        const {
            frecuenciaDias,
            fechaProximoCambio
        } = calcularFrecuenciaCambio(cantidadVacas, cantidadOrdeños, cantidadEquipos, fechaCambio);
